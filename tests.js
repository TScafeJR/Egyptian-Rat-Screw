...
const Card = require('./game/classes/card');
...
g.nextPlayer();
    ...
    it('should move the current player to the end of the array for 2 players', function() {
      const id1 = g.addPlayer('Ethan');
      const id2 = g.addPlayer('Josh');
      g.startGame();
...
g.slap('');
    ...
    it('should return an Object', function() {
      const e = g.addPlayer('Ethan');
      const j = g.addPlayer('Josh');
      g.startGame();
      g.playCard(e);
      g.playCard(j);
      g.playCard(e);
    ...
    it('should set game pile to an empty array if top of pile is Jack', function() {
      ...
      g.startGame();
      const ePile = ['hearts', 'spades'].reduce((cache, val) => {
        return [...cache, ...[...Array(13).keys()].map(i => new Card(val, i + 1))];
      }, []);
      const jPile = ['clubs', 'diamonds'].reduce((cache, val) => {
        return [...cache, ...[...Array(13).keys()].map(i => new Card(val, i + 1))];
      }, []);
    ...
    it('should slap successfully if the top card and third-to-top card are of the same value (sandwich)', function() {
      ...
      // swap king and queen
      const temp = jPile[jPile.length - 1]; // king
